...
public:
    ...
    Info solve(TreeNode* root, int& sum){
        ...
        currentAns.isBST =  (leftAns.maxVal < root -> val && rightAns.minVal > root -> val && leftAns.isBST && rightAns.isBST);

        //Jab bhi BST milega tabhi uska sum update karaunga
        //is taarike se mera pas max sum aajayega
        if(currentAns.isBST){
